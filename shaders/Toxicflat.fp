...
void SetupMaterial(inout Material material)				
{
	...
	vec4 normalmapA = texture(normaltexture, GetNormalLiquidscrollposAt(tbn)) * 0.25;
	vec4 normalmapB = texture(normaltexture, GetNormalLiquidscrollnegAt(tbn)) * 0.25;
	vec4 normalmapbase = normalmapA + normalmapB;
	
	////generate diffuse base texture
	vec4 DiffuseyposL = texture(Diffuse, (normalmapbase.xy + GetLiquidscrollposAt(tbn)) * 0.25);//diffuse texture scroll positive and brightness 
	vec4 DiffuseynegL = texture(Diffuse, (-normalmapbase.xy + GetLiquidscrollnegAt(tbn)) * 0.25);//diffuse texture scroll negetive and brightness 
	vec4 DiffuseyposXL = texture(Diffuse, (normalmapbase.xy + GetLiquidscrollposAt(tbn)) * 0.11);//diffuse texture scroll positive and brightness 
	vec4 DiffuseynegXL = texture(Diffuse, (-normalmapbase.xy + GetLiquidscrollnegAt(tbn)) * 0.11);//diffuse texture scroll negetive and brightness 
	vec4 DiffuseglowyposXL = texture(Diffuse, (-normalmap.xy + GetLiquidscrollposAt(tbn)) * 0.25);//glow texture scroll positive and brightness 
	vec4 DiffuseglowynegXL = texture(Diffuse, (-normalmap.xy + GetLiquidscrollnegAt(tbn)) * 0.11);//glow texture scroll negetive and brightness 
...
vec2 GetTopdiffusescaleAt(mat3 tbn)
{																		
    return ParallaxMap(tbn) * 12.5;//change scale of top diffuse texture here 
}
...
vec2 GetbottomdiffusescaleAt(mat3 tbn)
{																			
    return ParallaxMap(tbn) * 5.0;//change scale of bottom diffuse texture here
}
