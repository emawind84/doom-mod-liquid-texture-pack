...
Material ProcessMaterial()
{
	...
	vec4 NormalmapA = texture(Diffusedistortion, GetNormalLiquidscrollposAt(ParallaxMap));
	vec4 NormalmapB = texture(Diffusedistortion, GetNormalLiquidscrollnegAt(ParallaxMap));
	vec4 Normalmapbase = (NormalmapA + NormalmapB) * 0.15;
	...
	vec4 Glowblend = clamp(Layerglow * (clamp(Diffusemasktex * 40.0,0.0,1.0)) - (Diffusemasktex * 0.55),0.0,1.0);
	...
	vec4 Brightmapmask = clamp(Diffusefinal + 0.6 - (clamp(Diffusemasktex * 20.0,0.0,1.0)) + (clamp(Glowblend * 2.0,0.0,1.0)),0.0,1.0);
...
vec3 GetBumpedNormal(mat3 tbn, vec2 parallaxMap)
{
	...
	vec3 Normalmapmaksed = Normalmap * clamp(Diffusemasktex * 2.0,0.0,1.0);//remove diffuse normalmap color based on layermask brightness
